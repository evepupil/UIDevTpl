...
export default async function CategoryPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: SearchParams }) {
  ...
  return (
    <main className="category-page">
      ...
      <section className="category-heading"><div><p className="section-eyebrow">{category.name.en}</p><h1>{category.name[locale]}</h1><p>{category.description[locale]}</p></div><span className="category-heading-icon" aria-hidden="true">/ /</span></section>
