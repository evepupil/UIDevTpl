...
export function ModelsBlock({
  ...
  const columns: readonly ResourceColumn<ModelResource>[] = [
    {
      ...
      render: (model) => (
        <div className="blackline-resource-name">
          <div className="blackline-resource-name__icon">
    ...
    },
    {
      id: "requests",
      header: "Requests",
      render: (model) => <span>{model.requests}</span>,
    },
    {
      id: "updated",
      header: "Updated",
      render: (model) => (
        <span className="blackline-muted">{model.updatedAt}</span>
      ),
    },
    {
      id: "actions",
      ...
      render: (model) => (
        <div className="blackline-row-actions">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => onStatus?.(`${model.name} opened`)}
          >
            View
          ...
          <Button
            ...
            onClick={() => onStatus?.(`Actions for ${model.name}`)}
          >
  ...
  return (
    <>
      ...
      />
      <Card role="region" aria-label="Model filters">
      ...
      <Card className="gap-0" role="region" aria-label="Models list">
        <ResourceTable
          rows={rows}
          columns={columns}
